...
4. Build a complete runnable indicator surface:
   ...
   - write custom functions as `fn(bar_data, **kwargs)` over NumPy arrays that return a full-length one-dimensional array with NaN warmup bars, JIT-compiling explicit loops with a nested Numba `@njit` kernel
   ...
   - pass `pybroker.hyperparam` values as indicator kwargs for parameter search, and declare `add_execution(..., intervals=["weekly"])` for multi-timeframe indicators read with `ctx.interval("weekly").indicator("name")` (`timeframe=` is then required on `backtest`/`walkforward`)
...
- Vectorize first: write indicator logic with NumPy over `BarData` arrays, prefer the vectorized helpers (`highv`, `lowv`, `sumv`, `returnv`, `cross`, `atr`) when they fit, and JIT-compile explicit loops with a nested Numba `@njit` kernel that takes plain NumPy arrays — `BarData` cannot cross the `@njit` boundary. Never use pandas in your own indicator logic or in per-bar execution functions.
...
- For multi-timeframe indicators, do not pass an interval to `indicator()`; declare `add_execution(..., intervals=["weekly"])` so every indicator on that execution is also computed per interval, and read it with `ctx.interval("weekly").indicator("name")` over completed compressed bars. `backtest`/`walkforward` then require `timeframe=`, and each interval must be strictly coarser than the base timeframe.
