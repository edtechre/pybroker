...
- `strategy.set_max_long_positions(n)`/`set_max_short_positions(n)` accept an int greater than 0, a searchable `Hyperparam`, or `None` for unlimited. The `StrategyConfig` fields of the same names are deprecated and the setters take precedence; the bundled `wiki-10` page predates the setters, so always modernize its `StrategyConfig(max_long_positions=2)` pattern.
