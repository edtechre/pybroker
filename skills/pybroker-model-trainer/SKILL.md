...
- For models bound to a longer time interval, `lookahead` is measured in that interval's compressed bars, and predictions are read with `ctx.interval("...").preds(name)`.
...
- Vectorize first: write indicators as vectorized NumPy over `BarData` arrays, using Numba `@njit` for explicit loops, and read `ctx.*` NumPy arrays in execution functions. Do not use pandas inside indicator or execution functions; pandas belongs only at the `train_fn`/`input_data_fn` boundary where PyBroker hands you DataFrames.
