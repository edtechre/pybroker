...
class PriceScopeFetch:
    """V8-specific: exercise PriceScope.fetch with every PriceType.

    The generic walkforward only calls PriceScope.fetch on order
    placement (a fraction of bars), so the V8 optimizations (fast round,
    enum-value cache) barely register. This micro-bench drives fetch in
    a tight loop so the round + enum-dispatch cost dominates — then V8's
    delta stands out.
    """
    ...
    def setup(self) -> None:
        ...
        df = pd.DataFrame(
            {
                ...
                "high": np.full(252, 105.5),
                "low": np.full(252, 99.1),
                "close": np.full(252, 103.33),
            }
        ).set_index(["symbol", "date"])
        self._scope = PriceScope(ColumnScope(df), {"SPY": len(df)}, True)
        self._price_types = (
            PriceType.OPEN,
            PriceType.HIGH,
            PriceType.LOW,
        ...
        # Warm internal column caches so the timed method measures
        # only fetch + round, not first-hit ColumnScope slicing.
        for pt in self._price_types:
            self._scope.fetch("SPY", pt)

    def time_fetch_all_price_types(self) -> None:
        fetch = self._scope.fetch
        price_types = self._price_types
        iters = self.N // 6
